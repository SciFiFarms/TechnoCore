...
alias run_home_assistant='docker exec -it $(docker service ps -f desired-state=running --no-trunc ${stack_name}_home_assistant | grep ${stack_name} | tr -s " " | cut -d " " -f 2).$(docker service ps -f desired-state=running --no-trunc ${stack_name}_home_assistant | grep ${stack_name} | tr -s " " | cut -d " " -f 1) /bin/sh'
