...
alias run_fluentd='docker exec -it $(docker service ps -f desired-state=running --no-trunc ${stack_name:-technocore}_fluentd | grep ${stack_name:-technocore} | tr -s " " | cut -d " " -f 2).$(docker service ps -f desired-state=running --no-trunc ${stack_name:-technocore}_fluentd | grep ${stack_name:-technocore} | tr -s " " | cut -d " " -f 1) /bin/sh'
